...
public class InMemoryTaskRepository implements TaskRepository {
	...
	private final Map<Long, List<ActivityLogDto>> activityLogStore = new ConcurrentHashMap<>();
	private final Map<Long, List<CommentDto>> commentStore = new ConcurrentHashMap<>();
	...
	public void addActivity(Long taskId, String message) {
		ActivityLogDto log = new ActivityLogDto();
		log.setTaskId(taskId);
		log.setMessage(message);
		log.setTimestamp(System.currentTimeMillis());
		activityLogStore.computeIfAbsent(taskId, k -> new ArrayList<>()).add(log);
	}

	public void addComment(Long taskId, CommentDto comment) {
		commentStore.computeIfAbsent(taskId, k -> new ArrayList<>()).add(comment);
	}

	public List<CommentDto> getComments(Long taskId) {
		return commentStore.getOrDefault(taskId, List.of());
	}

	public List<ActivityLogDto> getActivityLogs(Long taskId) {
		return activityLogStore.getOrDefault(taskId, List.of());
	}
	...
	public List<TaskManagement> findByReferenceIdAndReferenceType(Long referenceId, ReferenceType referenceType) {
		return taskStore.values().stream().filter(
				task -> task.getReferenceId().equals(referenceId) && task.getReferenceType().equals(referenceType))
				.collect(Collectors.toList());
	}

	@Override
	public List<TaskManagement> findByAssigneeIdIn(List<Long> assigneeIds) {
		return taskStore.values().stream().filter(task -> assigneeIds.contains(task.getAssigneeId()))
				.collect(Collectors.toList());
	}
}
